...
//		run("Convert to Mask");
		...
		run("Analyze Particles...", "size=50-1500 circularity=0-0.80 show=Masks display clear include summarize add");  // circularity is set very stringent, so that only "dead cells" are counted

		saveAs("Tiff", file + "_mask_noncircular.tiff");
		
		close ("*");
	}		
}
Table.save(filepath + "Summary_GFP_non-circular.csv","Summary");	