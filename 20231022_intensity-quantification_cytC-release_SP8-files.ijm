...
for (i = 0; i < filelist.length; i++) {
	...
	if (matches(filename, your_string)) {	
		...
		// maybe needs to convert to 8-bit
		run("8-bit");
		
		// make sure the pixels are square
		run("Set Scale...", "distance=12.3072 known=1 unit=micron global"); //for whatever stupid reason, mito analyzer needs "micron" and not "µm" as Einheit!
		//run("Properties...", "channels=1 slices=1 frames=1 pixel_width=0.0812532 pixel_height=0.0812532 voxel_depth=1.0000000 global");
		...
		second_channel = replace(file, "_ch01", "_ch00");
		open(second_channel);
		title2 = getTitle();
		print(title2);
