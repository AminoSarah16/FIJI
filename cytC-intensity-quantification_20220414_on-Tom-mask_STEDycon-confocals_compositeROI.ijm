...
for (i = 0; i < subDirlist.length; i++) {
	...
	for (j = 0; j < filelist.length; j++) {
		...
		if (matches(filename, your_string)) {	
			...
			run("Analyze Particles...", "size=0.20-Infinity show=Masks add composite");  //do not check options for display nor clear results table
			//checking composite makes that it does notinclude holes within the masks
	...
	//		run("Measure");
			...
			run("Flatten");
			// and save it with the overlayed mask flattened
			save_title3 = replace(title2, ".tif", "_with-mask_flat.tif");
